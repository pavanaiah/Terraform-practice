resource "aws_vpc" "r_vpc" {
  ...
  provisioner "local-exec" {
    command = "echo terraform vpc has created >>log.txt"
...
resource "aws_s3_bucket" "r_s3_bucket" {
  bucket = "pavan-bucket4821"
  tags = {
    Environment = "dev"
  }
}
...
provisioner "local-exec" {
  command = "echo ${self.public_ip}instance has completed >> log.txt"
  
}

}

resource "aws_ebs_volume" "r_ebs" {

  availability_zone = aws_subnet.r_sub[0].availability_zone
  size = 15
  type = "gp3"
  tags = {
    Name = "secondary_vol_instance1"
  }
}


resource "aws_volume_attachment" "r_ebs_attch" {
  device_name = "/dev/sdh"
  volume_id = aws_ebs_volume.r_ebs.id
...
resource "null_resource" "ser_commands" {
    ...
    provisioner "file" {
      ...
      inline = [ "sudo chmod +x /tmp/nginx.sh" ,
                    "bash /tmp/nginx.sh"]
      
    }
  
}
resource "null_resource" "localprov" {
  triggers = {
    l_buildnum=1.0
  }
